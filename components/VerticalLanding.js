...
export default function VerticalLanding({ config }) {
  ...
  return <main className="mua-page"><Navbar hideLinks /><DianaChat vertical={config.slug} />
    ...
    <section id="mua-features" className="mua-section"><div className="mua-wrap"><div className="mua-section__heading"><p>Apa yang didapat</p><h2>Semua yang dibutuhkan untuk menerima booking lebih rapi.</h2></div><div className="mua-feature-grid">{featureRows.map(([icon, title, body]) => <article className="mua-feature glass" key={title}><span>{icon}</span><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>

    <section className="mua-section mua-package-summary"><div className="mua-wrap"><div className="mua-package-summary__card"><div><p className="mua-kicker">PAKET WEBSITE MUA</p><h2>Rp1.500.000</h2><p className="mua-muted">Satu paket untuk membuat bisnis MUA terlihat lebih profesional dan siap menerima calon client dari Instagram.</p><button onClick={openDiana} className="mua-button mua-button--primary">Tanya Chat Diana <b>→</b></button></div><div><p className="mua-package-summary__label">SUDAH TERMASUK</p><ul>{(config.packageIncludes || []).map((item) => <li key={item}><span>✓</span>{item}</li>)}</ul></div></div></div></section>
    ...
    {config.slug !== "mua" && <div className="mua-wrap"><BookingDemo config={config} /><CalendarPreview config={config} /></div>}
