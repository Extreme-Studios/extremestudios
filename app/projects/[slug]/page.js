...
export default function ProjectDetail({ params }) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-8 md:px-8">
      <a href="/#projects" className="inline-flex font-bold text-blue-300 hover:text-blue-200">
        Back to portfolio
      </a>

      <section className="grid min-h-[82vh] items-center gap-10 py-12 md:grid-cols-[0.85fr_1.15fr]">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.28em] text-blue-300">{app.category}</p>
          ...
          <a href="https://wa.me/6289677523666" target="_blank" rel="noreferrer" className="mt-9 inline-flex rounded-xl bg-primary px-6 py-3 font-black text-white shadow-glow transition hover:bg-blue-400">
            Bahas project sejenis
          </a>
        </div>
      ...
      <section className="grid gap-5 pb-20 md:grid-cols-4">
        {app.points.map((point, index) => (
          <article key={point} className="glass rounded-2xl p-6">
            <span className="text-xs font-black uppercase tracking-[0.24em] text-violet-200">Module {String(index + 1).padStart(2, "0")}</span>
